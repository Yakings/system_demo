...
class Canvases:
    def __init__(self, master=None,window_canvas=None):
        ...
        # self.a.axis((0, 3000, 0, 3000))
        self.a.set_xlim(0,3000)
        self.a.set_ylim(0,3000)
        self.line = self.a.plot([0, 0], [4000, 4000], 'b')[0]

        # self.line.set_xdata([0,1000])
        # self.line.set_ydata([0,1000])
    def set_x(self,x):
...
class Application():
    def __init__(self, master=None):
        ...
        # self.line = self.canvases.a.plot([0, 0], [4000, 4000], 'b')[0]
    ...
    def matplotlib_thread(self,root):
        if self.running == False:
            ...
            print("load model time:", load_model_time-start)
    ...
    def drawPic(self, canvases, x=t, y=s, add_model=False):
        ...
        while True:
            ...
            # if True:
                ...
                # canvases.a.set_xlim(0, 3000)
                # canvases.a.set_ylim(0, 3000)
                # canvases.a.set_title('Demo: Draw N Random Dot')
                # canvases.canvas.show()
                x_len = lenthy + 200
                y_len = max(y_sub + [0]) + 200
                self.canvases.set_x(x_sub)
                self.canvases.set_y(y_sub)
                self.canvases.a.set_xlim(0, x_len)
                self.canvases.a.set_ylim(0, y_len)
    ...
    def drawPic_onece(self, canvases, x=t, y=s, add_model=False):
        ...
        # y_len = 3000
        # canvases.a.axis((0,x_len,0,y_len))
        # canvases.set_x([0,100])
        # canvases.set_y([0,100])
        x_len = len(x_sub) + 200
        y_len = max(y_sub + [0]) + 200
        # print(y_len)
        self.canvases.set_x(x_sub)
        self.canvases.set_y(y_sub)
        canvases.a.set_xlim(0, x_len)
        canvases.a.set_ylim(0, y_len)
        # canvases.canvas.show()
        if len(y) > 1:
            show_str = 'Time:' + str(x[-1]) + ' RUL:' + str(round(y[-1], 2))
            self.rul_strvar.set(show_str)
        # end = time.time()
        # print('plot once time')
    ...
    def inference_run(self, data_name,predict_file_queue):
        ...
        while self.running:
            print('in inference, queue length is:', len(predict_file_queue.queue))
                ...
                if self.running:
                    ...
                    if self.auto_updating:
                        ...
                        try:
                            ...
                            # self.auto_updating_strvar.set('关闭自动')
                            self.update_strvar.set('无法连接云端')
                    time.sleep(0.5)
                    # print('data producer---copy:',origin_files[i])
                else:
                    break
            time.sleep(0)
    ...
    def creatTitle(self, name,position=[100, 220]):
        ...
        self.plottitle = Label(self.master,text=name,font=20)
    ...
    def creat_canvas(self):
        ...
        pass
    def creat_button_image(self):
        imagepaht2 = r'./button.jpg'
        self.button_image = ImageTk.PhotoImage(file=imagepaht2)

    def createWidgets(self):
        color_button = '#%02x%02x%02x' % (12, 92, 189)
        # color_button = [14.0/255,92.0/255,190.0/255]
        self.creat_button_image()

        self.rul_strvar = StringVar()
        self.rul_strvar.set('Time:0    RUL:0.0  ')
        self.show_rul = Label(self.master, textvariable=self.rul_strvar, font=20)
        self.window_canvas.create_window(650, 240, window=self.show_rul)


        self.update_strvar = StringVar()
        self.update_strvar.set('操作状态显示')
        self.show_update_strvar = Label(self.master, font=20,textvariable=self.update_strvar)
        self.window_canvas.create_window(950, 100, window=self.show_update_strvar)


        self.running_strvar = StringVar()
        self.running_strvar.set('诊断任务已停止')
        self.show_running = Label(self.master, textvariable=self.running_strvar, font=20)
        self.window_canvas.create_window(450, 240, window=self.show_running)


        self.begin_Plot_Button = Button(self.master,compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                        text='开始诊断任务',font=20,fg='white',
                                        activeforeground='white',
                                        activebackground='yellow', command=lambda:self.matplotlib_thread(self.master))
        self.window_canvas.create_window(100, 100,window=self.begin_Plot_Button)


        self.stop_Plot_Button = Button(self.master,compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                       font=20,fg='white',text='停止诊断任务',
                                       activeforeground='white',
                                       activebackground='yellow',command=lambda:self.stop_running_flag())
        self.window_canvas.create_window(100, 150, window=self.stop_Plot_Button)


        self.tasksetButton = Button(self.master, compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                    font=20,fg='white',text='诊断任务选择',
                                    activeforeground='white',
                                    activebackground='yellow',command=self.task_select)
        self.window_canvas.create_window(230, 100, window=self.tasksetButton)


        self.tasksetButton = Button(self.master,compound=CENTER,bg=color_button,image=self.button_image,relief=FLAT,
                                    font=20,fg='white',text='显示任务选择',
                                    activeforeground='white',
                                    activebackground='yellow',command=self.show_select)
        self.window_canvas.create_window(230, 150, window=self.tasksetButton)

        self.tasknewButton = Button(self.master, compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                    font=20,fg='white',text='  创建新任务 ',
                                    activeforeground='white',
                                    activebackground='yellow',command=self.new_task)
        self.window_canvas.create_window(360, 100, window=self.tasknewButton)

        self.begin_Plot_Button = Button(self.master, compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                        font=20,fg='white',text='清空诊断记录',
                                        activeforeground='white',
                                        activebackground='yellow',command=lambda:self.clear_result())
        self.window_canvas.create_window(360, 150, window=self.begin_Plot_Button)


        self.piccloseButton = Button(self.master,compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                     font=20,fg='white',text='云端查询模型',
                                     activeforeground='white',
                                     activebackground='yellow',command=self.open_search)
        self.window_canvas.create_window(570, 100, window=self.piccloseButton)

        self.localmodelButton = Button(self.master, compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                       fg='white',font=20,text='本地模型选择',
                                       activeforeground='white',
                                       activebackground='yellow',command=self.open_pbfile)
        self.window_canvas.create_window(570, 150, window=self.localmodelButton)


        self.auto_updating_strvar = StringVar()
        self.auto_updating_strvar.set('开启自动模式')
        self.auto_updating_Button = Button(self.master, compound=CENTER,bg=color_button,
                                           fg='white',font=20,image=self.button_image,
                                           relief=FLAT,textvariable=self.auto_updating_strvar,
                                           activeforeground='white',
                                           activebackground='yellow',command=self.change_auto_updating)
        self.window_canvas.create_window(700, 100, window=self.auto_updating_Button)

        self.update_model_Button = Button(self.master, compound=CENTER,bg=color_button, image=self.button_image,relief=FLAT,
                                          fg='white',font=20,text='   更新模型   ',
                                          activeforeground='white',
                                          activebackground='yellow',command=self.update_model_threading)
...
############################################################################################
    ...
    def user_login(self):
        ...
        self.user_show = Label(self.master, text='用户：', font=20,bg='black',fg='white')
        self.window_canvas.create_window(100, 50, window=self.user_show)

        self.user_name_show = Label(self.master, textvariable=self.user_var , font=20)
        self.window_canvas.create_window(150, 50, window=self.user_name_show)
        color_button = '#%02x%02x%02x' % (120, 92, 189)
        self.logoutButton = Button(self.master, font=20,text='更换用户',
                                   bg=color_button,
        ...
        self.window_canvas.create_window(860, 50, window=self.logoutButton)


        self.new_user_Button = Button(self.master, font=20,text='新建本地用户',
                                      bg=color_button,
                                      # image=self.button_image,
                                      fg='white',
                                      activeforeground='white',
                                      activebackground=color_button,
                                      command=self.creat_user)
    ...
    def new_task(self):
        ...
        path_var = StringVar()
        def selected_path(parent):
            origin_path = askdirectory(parent=parent)
        ...
        Button(self.window_new_task,text='路径选择',command=lambda:selected_path(self.window_new_task )).grid(row=3,column=2)
        ...
        if self.running == False:
            for i in range(len(selected_task_list)):
                ...
                try:
                    ...
                    # print('clean error')
                    self.update_strvar.set('内存已经释放')

                if os.path.exists(predicted_vib_sig_log):
                    os.remove(predicted_vib_sig_log)
                if os.path.exists(result):
                    os.remove(result)
                if os.path.exists(result_smooth):
