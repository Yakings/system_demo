...
import numpy as np
try:
    from models.read_util.read_train_data import get_all_bearings,get_all_bearings_list
except:
    from ..read_util.read_train_data import get_all_bearings,get_all_bearings_list
    # from read_train_data import get_all_bearings,get_all_bearings_list
...
def read_from_string_func(name,input_dim):
    ...
    # print(vib.dtype.name)
    return freq
def distorted_inputs(data_dir, batch_size=32, fix_size=True,input_dim=10):
    ...
    Returns:
      ...
      labels: Labels. 1D tensor of [batch_size] size.
    """
    filename_list, label_list = get_all_bearings_list(data_dir)
    ...
    with tf.name_scope('data_augmentation'):
        ...
        vibration = tf.py_func(read_from_string_func, [read_input.uint8image,input_dim], tf.float32)
        ...
        if fix_size:
            ...
            vibration.set_shape([input_dim,1])
        else:
...
def test_read_from_string_func(name,input_dim):
    # 旋转角度范围
    name = np.reshape(name, [1,])
    # print('name',name)
    csv_reader = csv.reader(open(name[0]))
    ...
    # print(vib.dtype.name)
    return freq
def test_inputs(data_dir, batch_size=32, fix_size=True,input_dim=10):
    """Construct distorted input for CIFAR training using the Reader ops.
    ...
    with tf.name_scope('data_augmentation_test'):
        ...
        vibration = tf.py_func(test_read_from_string_func, [read_input.uint8image,input_dim], tf.float32)
