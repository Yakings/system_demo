...
try:
    from models_mse_loss.read_util import test_data_producer as data_input
    from models_mse_loss import resnet_con1d as resnet
except:
    from read_util import test_data_producer as data_input
    import resnet_con1d as resnet
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope('train_image'):
                # train_images, train_labels = data_input.distorted_inputs(FLAGS.train_image_root, FLAGS.train_dataset, FLAGS.batch_size, True, num_threads=num_threads, num_sets=FLAGS.num_gpus)
            ...
            with tf.variable_scope('test_image'):
                val_images, val_labels = data_input.distorted_inputs(FLAGS.batch_size)
        ...
        for step in range(init_step, FLAGS.max_steps):
            ...
            if step % FLAGS.val_interval == 0:
                ...
                for i in range(FLAGS.val_iter):
                    start = time.time()
                    loss_value, acc_value, preds_value, _labels_value = sess.run([network_val.loss, network_val.acc,network_val.logits,network_val._labels],
                                feed_dict={network_val.is_train:True})
                    val_loss += loss_value
                    val_acc += acc_value
                    import numpy as np
                    _labels_value = np.squeeze(_labels_value)
                    # print('preds:', np.shape(preds_value))
                    # print('label:', np.shape(_labels_value))
                    print('preds:', preds_value)
                    print('label:', _labels_value)
                    mse = np.sqrt(np.mean(np.square((preds_value-_labels_value))))
                    end = time.time()
                    one_time = end-start
