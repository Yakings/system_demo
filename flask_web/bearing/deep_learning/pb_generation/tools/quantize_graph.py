...
def quantize_graph(input='_optimized.pb',
                   output_node_names='_/Softmax',
                   output='_quaintized.pb',
                   mode='weights_rounded',
                   ##logtostderr='',
                   quantized_input=False,
                   quantized_fallback_min=None,
                   quantized_fallback_max=None,
                   quantized_input_min=0,
                   quantized_input_max=1):
  ...
  if not any(mode in s for s in known_modes):
    print("mode is '" + mode + "', not in " + ", ".join(known_modes) +
          ".")
    return -1
  ...
  if quantized_input:
    quantized_input_range = [
        quantized_input_min, quantized_input_max
    ]

  fallback_quantization_range = None
  if (quantized_fallback_min is not None or
      quantized_fallback_max is not None):
    assert quantized_fallback_min is not None
    assert quantized_fallback_max is not None
    fallback_quantization_range = [
        quantized_fallback_min, quantized_fallback_max
    ]

  rewriter = GraphRewriter(tf_graph, mode, quantized_input_range,
                           fallback_quantization_range)

  output_graph = rewriter.rewrite(output_node_names.split(","))

  f = gfile.FastGFile(output, "wb")
  f.write(output_graph.SerializeToString())
